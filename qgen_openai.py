...
class QGenOpenai:
    def __init__(self, content, questions_number: int = 5) -> None:
        ...
        self.mcq_function = self._create_mcq_function()

    def _create_mcq_function(self):
        return [
            {
                "name": "create_mcq",
                "description": f"Create {self.questions_number} multiple choice questions from the input text with four candidate options. Three options are incorrect and one is correct. Indicate the correct option after each question.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "questions": {
